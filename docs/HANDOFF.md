...
## Next action (REBUILD — RB7)

**RB6 is COMPLETE & committed (`dd8a23e`).** Next is **RB7 — Reports & export**: (1) `domain/exportLegacy.ts` — `exportLegacyRawData(project, ttId)` emitting the legacy text the OLD viewer parses (DATA_MODEL § "Legacy export": `Monday\nClass,Period 1..N\nClass 1,Maths (Bindu),...`; multi-teacher ` / `; empty `Free`; blocks expand per period per class; teaching slots become Period 1..8) — with a round-trip test reconciling against `deriveMaps`; (2) `domain/reports.ts` — workload, subject-count, free-period, clash reports, each reconciling with `deriveMaps`/`validate` (a property test); reuse `insights.ts` where possible; (3) UI — class/teacher/day printable sheets (add a whole-school DAY view) + a Reports surface + a "Download timetable file" button for the legacy export. AC: the three sheets visually match the source PDFs (owner-side visual check, standard caveat); reports reconcile with derive(); legacy export round-trips. Then **RB8 — Operational tools** (substitution planner, "what if teacher X leaves", named versions/compare, mobile teacher view, PWA offline).
