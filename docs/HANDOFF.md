...
## Next action (REBUILD — start RB3)

**RB2 is COMPLETE** (committed: RB2 1/n..4/n). Legal-only editor end-to-end: click → picker (only legal options) with Explain + shared-cell guard + "Suggested" ghost + "Swap with" list; class/teacher views share one store; class-health dot + teacher load bar; global undo; keyboard-accessible cells; mobile-readable; drag-with-auto-swap + ghost autocomplete. Tests: picker independent-oracle both sides, swap/move two-sided + HE4/HE1/HE7 negatives, ghost legal+deterministic, edit isolation, cross-view store (50 tests).
