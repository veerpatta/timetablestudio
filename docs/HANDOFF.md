...
## Next action (REBUILD — RB8, the final milestone)

**RB7 is COMPLETE & committed (`c678f13`).** Next is **RB8 — Operational tools** (the last milestone): (1) substitution planner — `domain/substitute.ts`: covers for an absent teacher's lessons = teachers free at that slot (insights.freeTeachers) AND qualified for every (subject,class) of the event; "what if teacher X leaves" = the same over their whole week; test: covers are free+qualified+available; (2) named versions — store `versions[]` with save/restore + `domain/diffTimetables.ts` (pure cell diff) + test (compare & restore); (3) PWA offline — manifest + service worker + registration (the numeric "installable offline" Lighthouse check is owner-side, standard caveat); (4) mobile teacher view (the existing teacher view is already responsive/overflow-auto; add a compact card view if time). AC: marking a teacher absent yields valid covers respecting availability; versions compare and restore; installable offline. After RB8: merge `rebuild`→`main` is OWNER-gated (do NOT auto-merge; `main` auto-deploys over the live app).
