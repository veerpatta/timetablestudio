...
**All RB0–RB8 merged to `main` (`b13666e`) and deployed.** There is no next milestone. Remaining items are the owner's, none blocking:
...
2. **Supply a current 6-period heatwave timetable** if the switchable heatwave profile is wanted (the only kickoff bullet not delivered — mechanism ready, current data pending; the in-repo `classWisePdf.ts` is pre-pivot M18 data, not the 2026-27 heatwave; see DECISIONS RB8 for the fold recipe).
