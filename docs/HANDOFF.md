...
## Next action (REBUILD — finish RB5, then RB6)

**RB5 core is done & committed (`3d83c4a`); the pure solver + Node AC test pass.** Finish RB5 with **Task #9**: (1) a thin **Web Worker** wrapper `solver/fillWorker.ts` that marshals `fill(project, ttId, {seed})` in/out (the FIRST worker of the rewrite — AGENTS §1 requires the solver run off the main thread; the AC test still calls the pure `fill()` directly in Node, never the worker); (2) a **"Fill the gaps"** button that runs the solver and shows the returned `FilledPlacement[]` as a **reviewable accept/reject diff** (nothing silent — apply via `store.applyFix` so it's undoable; reject discards). Surface the seed and honestly show any remaining gaps the greedy pass couldn't fill. Reuse the existing diff/undo pattern (`applyFix`). Live-verify (the screenshot tool has been timing out; verify via DOM eval + a jsdom integration test like `App.issues.test.tsx`).

**Then RB6 — Rules library** (configurable sentence-first rules R1–R24 evaluated on the event model, "suggest rules" detector). See REBUILD.md.
