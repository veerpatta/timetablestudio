...
## Next action (REBUILD — start RB5)

**RB4 is COMPLETE** (committed `aaa6a3f`). Next is **RB5 — Lock & auto-fill generator** (the solver, in a Web Worker per AGENTS §1 — first worker of the rewrite). Place most-constrained first (Mahesh, Anjana), then pinned blocks + joint events, then fill remaining holes; present the result as an accept/reject DIFF (nothing silent); deterministic per seed. AC: clearing ~30% of non-pinned slots and filling reaches 0 real clashes in < 5 s in a Node test; pinned/joint/team events are NEVER moved. Reuse `validate()` as the feasibility oracle and `legalOptions`/`canMove` for candidate generation; HE8 (pinned immovable) is enforced here. Solver must be pure + deterministic(seed) and live in `solver/` behind a worker wrapper.
