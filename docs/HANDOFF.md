...
## Next action (REBUILD — RB6)

**RB5 is COMPLETE & committed (`427a69f`).** Next is **RB6 — Rules library**: the R1–R15 catalog is already TYPED in `types.ts` (carried to v6 with slot terms) and `Project.rules` exists but `validate()` does not yet evaluate them. Build: (1) `domain/rules.ts` — `evaluateRules(project, tt): Violation[]` dispatching per template (must→hard, prefer→soft), integrated into `validate()` (bundled has `rules: []` so it stays 0-issue); (2) `domain/ruleText.ts` — `ruleSentence(rule, project)` plain-language for every template (no codes on the main surface); (3) `suggestRules(project, tt)` — detect ≥3 real patterns (class-teacher P1 anchors, double periods, block days, teacher caps), no duplicates, one-click add; (4) UI — sentence-first rule toggles + add-from-suggestions; violations flow through the existing Issues panel. AC: each shipped template has satisfied+violated tests with plain messages; suggestions ≥3, no dupes. NOTE: R16–R24 are doc-only (not in types.ts) — ship R1–R15; note R16–R24 as deferred. R7 is structural (allowedDays/fixedStart aren't on the v6 event) — implement as a placement-consistency check (all placements of the event share one start slot) and record the interpretation in DECISIONS.
