...
## Next action (REBUILD — RB1, BLOCKED on owner-provided grid)

**RB1 — Real 2026-27 timetable as bundled default.** Build the bundled v6 project: profiles (regular 8-period default + heatwave secondary — both DONE in `domain/profile.ts`), the 16 classes, the 19 teachers (schedulable flags; Mahesh narrow + Anjana P5–P8 availability), band-scoped subjects (Science = 3 subjects middle / 1 secondary), qualifications, requirements, and the placed events (normal + ELGA team_block + senior joint English/Hindi/Economics + special slots). AC: app opens to it with 0 real clashes; joint/team render as single events; a fixture test matches the source views cell-for-cell.

**⚠ BLOCKER (owner-decided 2026-06-07):** confirmed empirically that EVERY in-repo cell-level source — `docs/sources/Class_Wise.pdf`, `Day_Wise.pdf`, `Teacher_Wise Copy.pdf`, `rawData.vpps.txt`, and `fixtures/classWisePdf.ts` — is the **6-period HEATWAVE** timetable. The 8-period regular 2026-27 grid exists only as *structural description* in the two analysis docs (skeleton, ELGA, joint classes, qualifications, availability, weekly counts) — there is **no cell-by-cell 8-period grid** to transcribe or match against. **Owner chose: "You'll give me the 8-period grid"** (AskUserQuestion). So RB1's grid-derived parts WAIT for the owner's file rather than being inferred from 6-period data (the grid is the authoritative source for placements AND for confirming the qualification table).

**Exact next step when the grid arrives** (all 16 classes × 6 days × 8 periods, each cell = subject + teacher, plus which slots are ELGA / joint / free; any format):
1. Transcribe into `fixtures/` as a cell grid (the cell-for-cell fixture), like the old `classWisePdf.ts` but 8-period.
2. Build `fixtures/bundled.ts` → `buildBundledProject()`: entities (classes/teachers/subjects/rooms), qualifications derived from the grid's (teacher, subject, class) pairs, requirements from weekly counts, then events (normal/joint_class/team_block/special) + placements onto the regular profile.
3. Assert `validate()` returns 0 hard violations and the grid round-trips cell-for-cell.

**Done while blocked (this session, zero-rework, grid-independent):** the heatwave 6-period secondary profile (`buildHeatwaveProfile`, clock from Day_Wise.pdf, + test). Did NOT speculatively build entities/qualifications — those are most-correctly derived from the authoritative grid the owner will supply.
