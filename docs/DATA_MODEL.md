...
`TimetableEvent` gains `studentGroupIds?: Id[]` — when present, the event is attended only by those student groups (an elective), not the whole class. **Clash rule extension:** a student group may not appear in two different events in the same slot; an opted-out group gets a `self_study`/`free` event during a dropped-elective line. Same-event overlap remains legal (joint/team). `Project` gains `constraints: Constraint[]` (added in C3), and will gain `electiveGroups: ElectiveGroup[]`, `studentGroups: StudentGroup[]` (C5). **`rules: Rule[]` was REMOVED in C4** — the `Rule`/`R*Rule` union, `evaluateRules`, `rules.ts`, `ruleText.ts`, `suggestRules.ts`, and the `Project.rules` field are all gone; `constraints` is the only system. The C4 catalog covers the R1–R15 semantics as constraint templates (plus more). A load-time migration in `normalizeProject` reads any legacy `rules` off a persisted blob, converts an R4 (class-teacher-P1) → `class_teacher_p1` constraint, and drops `rules` (never carried forward). The C4 constraint catalog (24 templates) is enumerated in `src/domain/constraintCatalog.ts`; templates needing absent model support are deferred (see DECISIONS C4).
